"use strict";
/*---------------------------------------------------------------------------------------------
 *  Copyright (c) Microsoft Corporation. All rights reserved.
 *  Licensed under the MIT License. See License.txt in the project root for license information.
 *--------------------------------------------------------------------------------------------*/
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
const vscode_1 = require("vscode");
const callWithTelemetryAndErrorHandling_1 = require("./callWithTelemetryAndErrorHandling");
const extensionVariables_1 = require("./extensionVariables");
const AzureTreeItem_1 = require("./treeDataProvider/AzureTreeItem");
// tslint:disable:no-any no-unsafe-any
function registerCommand(commandId, callback) {
    extensionVariables_1.ext.context.subscriptions.push(vscode_1.commands.registerCommand(commandId, (...args) => __awaiter(this, void 0, void 0, function* () {
        return yield callWithTelemetryAndErrorHandling_1.callWithTelemetryAndErrorHandling(commandId, function () {
            if (args.length > 0) {
                const contextArg = args[0];
                if (contextArg instanceof AzureTreeItem_1.AzureTreeItem) {
                    this.properties.contextValue = contextArg.contextValue;
                }
                else if (contextArg instanceof vscode_1.Uri) {
                    this.properties.contextValue = 'Uri';
                }
            }
            return callback.call(this, ...args);
        });
    })));
}
exports.registerCommand = registerCommand;
function registerEvent(eventId, event, callback) {
    extensionVariables_1.ext.context.subscriptions.push(event((...args) => __awaiter(this, void 0, void 0, function* () {
        return yield callWithTelemetryAndErrorHandling_1.callWithTelemetryAndErrorHandling(eventId, function () {
            return callback.call(this, ...args);
        });
    })));
}
exports.registerEvent = registerEvent;
//# sourceMappingURL=AzureActionHandler.js.map